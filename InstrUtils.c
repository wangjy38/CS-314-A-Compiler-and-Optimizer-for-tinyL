...
void PrintInstructionList(FILE * outfile, Instruction * instr)
{
	...
	/* YOUR CODE GOES HERE */
        while (instr!=NULL) {
                //fprintf(outfile, "%c ",'3');
                //fprintf(outfile, "%d ", instr->field1);
                //fprintf(outfile, "%d ", instr->field2);
                //fprintf(outfile, "%d ", instr->field3);
                //if (instr->critical=='M') fprintf(outfile, "%c ",'M');
		PrintInstruction(outfile, instr);
                //if (instr->critical=='M') fprintf(outfile, "%c",'1');
		instr = instr->next;
	}
}
